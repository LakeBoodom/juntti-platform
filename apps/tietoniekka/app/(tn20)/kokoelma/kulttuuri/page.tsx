...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const canonical = `${SITE_URL}/kokoelma/kulttuuri`;
  return {
    title, description,
    ...
    <main className="tnk" style={{ minHeight: "100dvh", paddingBottom: 60 }}>
      ...
      <div className="tn-shell">
        ...
        <nav className="tn-chipnav tnk-filters" aria-label="Alakokoelmat">
          <a href="/kokoelma/kulttuuri" data-active={filter === "kaikki" || undefined}>Kaikki</a>
          {presentSubs.map((s) => (
            <a key={s.key} href={`/kokoelma/kulttuuri?suodata=${s.key}`} data-active={filter === s.key || undefined}>
              {s.label}
            </a>
          ))}
        </nav>
